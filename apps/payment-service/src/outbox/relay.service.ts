...
import { PaymentChargedSchema, PaymentRefundedSchema } from '@app/contracts';
...
export class RelayService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    await this.kafka.registerSchema('payment-charged-value', PaymentChargedSchema);
    await this.kafka.registerSchema('payment-refunded-value', PaymentRefundedSchema);
